...
## WICHTIG: Konsistenz über Seiten hinweg

**Vor jeder neuen Feature-Implementierung prüfen, ob ähnliche Lösungen bereits existieren!**

Bei UI-Komponenten wie Lightboxen, Modals, Formularen, Animationen etc.:

1. **Zuerst suchen:** Prüfe ob eine ähnliche Komponente/Funktion bereits auf anderen Seiten existiert
2. **Benutzer fragen:** Falls ja, frage den Benutzer:
   > "Ich habe eine ähnliche Lösung auf [Seite X] gefunden, die [Framer Motion / Dialog / etc.] verwendet. Soll ich diese Implementierung übernehmen für Konsistenz?"
3. **Übernehmen:** Bei Freigabe die bestehende Lösung als Vorlage nutzen
4. **Dokumentieren:** Neue wiederverwendbare Patterns in `KNOW-HOW.md` festhalten

**Beispiele für konsistente Komponenten:**
- Lightboxen: Framer Motion Overlay mit AnimatePresence (siehe `portfolio.tsx`)
- Video Player: `VideoPlayer` Komponente aus `@/components/shared`
- Formulare: React Hook Form + Zod Validation
- Animationen: Framer Motion mit `containerVariants` / `itemVariants` Pattern

**Ziel:** Gleiche Funktionalität = Gleiche Implementierung auf allen Seiten
...
Diese Dokumentation ist die Single Source of Truth für technische Entscheidungen.<!-- BEGIN BYTEROVER RULES -->
